...
export class BMOSettingTab extends PluginSettingTab {
	...
	async display(): Promise<void> {
		...
		containerEl.createEl("h1", { text: "My BMO Chatbot Settings" });
		...
		containerEl.createEl("p", {
			text: "Type `/help` in chat for commands.",
		});
		...
		resetButton.addEventListener("click", async (event) => {
			...
			if (confirmReset) {
				const profilePathFile =
					this.plugin.settings.profiles.profileFolderPath +
					"/" +
				...
				if (profilePath) {
					if (profilePath.path === defaultProfilePath.path) {
						...
						// @ts-ignore
						await this.plugin.app.plugins.enablePlugin(
							this.plugin.manifest.id,
						);
					} else {
						const filenameMessageHistory =
							...
							"messageHistory_" +
							defaultProfilePath.name.replace(".md", ".json");
				...
				requestAnimationFrame(() => {
					// @ts-ignore
					const refreshTab =
						this.plugin.app.setting.openTabById("bmo-chatbot");
					if (refreshTab) {
						refreshTab.display();
					} else {
