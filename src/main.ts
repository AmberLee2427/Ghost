	...
	async onload() {
		...
		this.registerEvent(
			this.app.vault.on("delete", async (file: TFile) => {
				if (file instanceof TFile && file.path.startsWith(folderPath)) {
					const filenameMessageHistory =
						"./.obsidian/plugins/bmo-chatbot/data/" +
						"messageHistory_" +
						file.name.replace(".md", ".json");
					...
					if (file.path === defaultFilePath) {
						...
						if (this.settings.profiles.profile === file.name) {
							...
							const fileContent = (
								await this.app.vault.read(defaultProfile)
							)
								.replace(/^---\s*[\s\S]*?---/, "")
								.trim();
					...
					try {
						...
						if (
							...
							const filenameMessageHistoryPath =
								"./.obsidian/plugins/bmo-chatbot/data/";
							const oldProfileMessageHistory =
								"messageHistory_" +
								oldPath
									.replace(folderPath + "/", "")
									.replace(".md", ".json");
		...
		this.addRibbonIcon("bot", "My BMO Chatbot", () => {
			this.activateView();
		});

		this.addCommand({
			id: "open-bmo-chatbot",
			name: "Open My BMO Chatbot",
			callback: () => {
				this.activateView();
			},
			hotkeys: [
		...
		this.registerEvent(
			this.app.workspace.on("file-menu", (menu, file) => {
				...
				menu.addItem((item) => {
					item.setTitle("My BMO Chatbot: Generate new title").onClick(
						() => renameTitleCommand(this, this.settings),
					);
				});
			}),
...
export async function updateSettingsFromFrontMatter(
	...
	const updateSettings = async (frontmatter: any) => {
		...
		plugin.settings.appearance.botMessageBackgroundColor =
			"#" + frontmatter.chatbot_message_background_color;
		plugin.settings.appearance.chatBoxFontColor =
			"#" + frontmatter.chatbox_font_color;
			...
			(frontmatter: any) => {
				...
				if (modelName) {
					modelName.textContent =
						"Model: " + plugin.settings.general.model;
				}
				...
				if (
					...
					plugin.settings.appearance.chatbotContainerBackgroundColor =
						"#" +
						frontmatter.chatbot_container_background_color.substring(
							0,
							6,
						);
					if (chatbotContainer) {
					...
					plugin.settings.appearance.chatbotContainerBackgroundColor =
						colorToHex(
							DEFAULT_SETTINGS.appearance
								.chatbotContainerBackgroundColor,
						);
					frontmatter.chatbot_container_background_color =
						plugin.settings.appearance.chatbotContainerBackgroundColor.replace(
							/^#/,
					...
					if (chatbotContainer) {
						const defaultChatbotContainerBackgroundColor =
				...
				if (
					...
					plugin.settings.appearance.messageContainerBackgroundColor =
						"#" +
						frontmatter.message_container_background_color.substring(
							0,
							6,
						);
					if (messageContainer) {
					...
					plugin.settings.appearance.messageContainerBackgroundColor =
						colorToHex(
							DEFAULT_SETTINGS.appearance
								.messageContainerBackgroundColor,
						);
					frontmatter.message_container_background_color =
						plugin.settings.appearance.messageContainerBackgroundColor.replace(
							/^#/,
					...
					if (messageContainer) {
						const defaultMessageContainerBackgroundColor =
							getComputedStyle(document.body)
								.getPropertyValue(
									DEFAULT_SETTINGS.appearance
										.messageContainerBackgroundColor,
				...
				if (isValidHexColor(frontmatter.user_message_font_color)) {
					...
					if (messageContainer) {
						...
						userMessages.forEach((userMessage) => {
							const element = userMessage as HTMLElement;
							const defaultUserMessageFontColor =
								getComputedStyle(document.body)
									.getPropertyValue(
										DEFAULT_SETTINGS.appearance
				...
				if (
					isValidHexColor(frontmatter.user_message_background_color)
				) {
					plugin.settings.appearance.userMessageBackgroundColor =
						"#" +
						frontmatter.user_message_background_color.substring(
							0,
							6,
						);
					if (messageContainer) {
					...
					if (messageContainer) {
						...
						userMessages.forEach((userMessage) => {
							const element = userMessage as HTMLElement;
							const defaultUserMessageBackgroundColor =
								getComputedStyle(document.body)
									.getPropertyValue(
										DEFAULT_SETTINGS.appearance
											.userMessageBackgroundColor,
									...
									.trim();
							element.style.backgroundColor =
								defaultUserMessageBackgroundColor;
						});
					}
				}
				...
				if (isValidHexColor(frontmatter.bot_message_font_color)) {
					...
					if (messageContainer) {
						...
						botMessages.forEach((botMessage) => {
							const element = botMessage as HTMLElement;
							const defaultBotMessageFontColor = getComputedStyle(
								document.body,
							)
								.getPropertyValue(
									DEFAULT_SETTINGS.appearance
				...
				if (
					...
					plugin.settings.appearance.botMessageBackgroundColor =
						"#" +
						frontmatter.chatbot_message_background_color.substring(
							0,
							6,
						);
					if (messageContainer) {
					...
					if (messageContainer) {
						...
						botMessages.forEach((botMessage) => {
							const element = botMessage as HTMLElement;
							const defaultBotMessageBackgroundColor =
								getComputedStyle(document.body)
									.getPropertyValue(
										DEFAULT_SETTINGS.appearance
											.botMessageBackgroundColor,
				...
				if (isValidHexColor(frontmatter.chatbox_font_color)) {
					...
					const defaultChatBoxFontColor = getComputedStyle(
						document.body,
					)
						.getPropertyValue(
							DEFAULT_SETTINGS.appearance.chatBoxFontColor,
				...
				if (isValidHexColor(frontmatter.chatbox_background_color)) {
					...
					const defaultChatBoxBackgroundColor = getComputedStyle(
						document.body,
					)
						.getPropertyValue(
							DEFAULT_SETTINGS.appearance.chatBoxBackgroundColor,
				...
				plugin.settings.appearance.allowHeader =
					frontmatter.allow_header;
